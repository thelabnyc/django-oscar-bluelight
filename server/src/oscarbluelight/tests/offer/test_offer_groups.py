...
class ConsumeOfferGroupOfferTest(TestCase):
    ...
    @mock.patch(
        "oscar.apps.offer.abstract_models.AbstractConditionalOffer.apply_benefit"
    )
...
class OfferGroupApplicatorTest(TestCase):
    ...
    def test_apply_offers(
        ...
        offer_group_stooges = OfferGroup.objects.create(
            name="test offer group stooges", priority=1
        )
        ...
        pre_offers_apply.assert_called_once_with(
            sender=Applicator, basket=basket, offers=offers
        )
        post_offers_apply.assert_called_once_with(
            sender=Applicator, basket=basket, offers=offers
        )
        pre_offer_group_apply.assert_has_calls(
            [
                mock.call(
                    sender=Applicator,
                    basket=basket,
                    group=offer_group_stooges,
                    offers=[offer_stooges],
                ),
    ...
    def test_line_prices_never_become_negative(self):
        ...
        condition_require_one_product.proxy_class = (
            "oscarbluelight.offer.conditions.BluelightCountCondition"
        )
        ...
        condition_require_two_products.proxy_class = (
            "oscarbluelight.offer.conditions.BluelightCountCondition"
        )
        ...
        benefit_one_product_free.proxy_class = (
        ...
        benefit_300_dollars_off.proxy_class = (
            "oscarbluelight.offer.benefits.BluelightAbsoluteDiscountBenefit"
        )
    ...
    def test_offers_cannot_compound_within_a_group(self):
        ...
        self.assertTrue(
            offer_300_dollars_off.pk not in basket.offer_applications.applications
        )
...
class OfferGroupViewTest(TestCase):
    ...
    def test_get_list(self):
        ...
        self.assertEqual(
            response.context_data.get("offergroup_list")[1].name, "someName"
        )
    ...
    def test_update(self):
        ...
        resp_get = self.client.get(
            reverse("dashboard:offergroup-update", args=[self.offer_group.pk])
        )
        self.assertEqual(resp_get.status_code, 200)
        form = resp_get.context["form"]
        data = form.initial
