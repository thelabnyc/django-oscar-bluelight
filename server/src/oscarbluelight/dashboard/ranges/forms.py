...
class RangeSearchForm(forms.Form):
    ...
    def filter_queryset(self, qs):
        ...
        if data.get('text'):
            qs = qs.annotate(search=SearchVector('name', 'slug', 'description'))\
                .filter(search=data['text'])
            is_filtered = True
        ...
        if data.get('product_name') or data.get('upc') or data.get('sku'):
            ...
            if data.get('sku'):
                products_qs = products_qs.filter(stockrecords__partner_sku__iexact=data['sku'])
...
class BatchPriceUpdateForm(forms.Form):
    ABSOLUTE, PERCENTAGE_RETAIL, PERCENTAGE_ACTUAL = 'ABSOLUTE', 'PERCENTAGE_RETAIL', 'PERCENTAGE_ACTUAL'
    TYPES = (
    ...
    operation_type = forms.ChoiceField(
        label='Type of Change to Apply',
        initial=PERCENTAGE_RETAIL,
        choices=TYPES)
    ...
    def apply_discount(self, price_retail, price_excl_tax):
        ...
        operations = {
            ...
            self.PERCENTAGE_ACTUAL: lambda: price_excl_tax + (price_excl_tax * (amount / 100)),
